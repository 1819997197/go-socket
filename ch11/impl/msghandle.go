...
func NewMsgHandle() iface.IMsgHandle {
	return &MsgHandle{
		...
		WorkerPoolSize: conf.WorkPoolSize,
		TaskQueue:      make([]chan iface.IRequest, conf.WorkPoolSize),
	}
}
...
func (m *MsgHandle) StartWorkerPool() {
	for i := 0; i < int(conf.WorkPoolSize); i++ {
		// 一个worker对应一个queue
...
func (m *MsgHandle) SendMsgToTaskQueue(request iface.IRequest) {
	fmt.Println("SendMsgToTaskQueue connId: ", request.GetConn().GetConnID())
	// 根据connId取模来分配队列
	workId := request.GetConn().GetConnID() % conf.WorkPoolSize
